...
Es muy importante que antes de salir a producción realices pruebas del flujo completo, verificando que la creación de pagos se realice en forma correcta y validando que los mensajes sean efectivos a la hora de comunicar al usuario.

Una buena experiencia de tus clientes en el checkout ayuda a garantizar la conversión.

Cuentas con un par de credenciales de sandbox, que te permitián probar toda la integración en una réplica exacta del Modo Producción pudiendo simular transacciones utilizando las tarjetas de prueba:
...
En cada caso, es muy importante que comuniques a tu cliente el resultado del pago y qué debe hacer como próximo paso.
Para ello te informaremos con un HTTP Status 201 que el pago ha sido creado correctamente y enviaremos un [código de resultado] para que puedas redirigir al cliente a la página con el mensaje correcto. 

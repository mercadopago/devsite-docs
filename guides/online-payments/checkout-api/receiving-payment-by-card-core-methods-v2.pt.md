...
Nesse passo são criados os campos mais seguros, hospedados pelo Mercado Pago, e que fazem uso do elemento `HTML iframe`.

```javascript
// Step #createInputs

const cardNumberElement = mp.secureInputs.create('cardNumber', {
     placeholder: "Card Number",
     style: {
       // You add here some input styles. Check documentation at [DOCUMENTAÇÃO AQUI] to know more
       // Ex:
       // padding: "10px"
     }
...
const cardNumberElement = mp.secureInputs.create('cardNumber', {
   ...
   const expirationMonthElement = mp.secureInputs.create('expirationMonth', {
     placeholder: "MM",
     style: {
       // You add here some input styles. Check documentation at [DOCUMENTAÇÃO AQUI] to know more
     }
   }).mount('form-checkout-expirationMonth-container')
   const expirationYearElement = mp.secureInputs.create('expirationYear', {
     placeholder: "YYYY",
     style: {
       // You add here some input styles. Check documentation at [DOCUMENTAÇÃO AQUI] to know more
     }
   }).mount('form-checkout-expirationYear-container')
 
   const securityCodeElement = mp.secureInputs.create('CVV', {
     placeholder: "CVV",
     style: {
       // You add here some input styles. Check documentation at [DOCUMENTAÇÃO AQUI] to know more
     }
   }).mount('form-checkout-securityCode-container')
   ```
...
const getIssuers = async (bin) => {
     try {
       ...
       const issuers = await mp.getIssuers({ paymentMethodId: paymentMethodID, bin });
   ...
   const createCardToken = async (event) => {
     try {
       ...
       if (!tokenElement.value) {
         ...
         const token = await mp.secureInputs.createCardToken({

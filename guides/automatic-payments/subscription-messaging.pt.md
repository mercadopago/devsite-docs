...
> Importante
>
> No caso de operações com pagamentos recorrentes das bandeiras, será necessário enviar o identificador de transação da bandeira (TID) às transações de mensageria. Para mais informações sobe como enviar o TID, acesse a documentação [Network Transaction ID - TID](/developers/pt/guides/automatic-payments/network-transaction-id).
...
| first_time_use| boolean | Indica se é o primeiro pagamento da assinatura | 'true' ou 'false' |
...
| user_present | boolean | Indica se houve intervenção do usuário no momento da criação do pagamento |'true' ou 'false' |
...
| payer.type | string | Tipo de identificação do pagador associado | 'guest' ou 'customer' |

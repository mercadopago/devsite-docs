...
4. En la pantalla **Calidad de integración**, haz clic en **Evaluar calidad** e ingresa el `payment ID` de un pago reciente realizado con [credenciales](/developers/es/guides/additional-content/credentials/credentials) de producción de la aplicación que estás queriendo homologar. Siempre que sea posible, ingresa en el campo el último `payment ID` productivo que identifiques en esa aplicación.
